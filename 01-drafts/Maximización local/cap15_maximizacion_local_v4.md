...
Esta es exactamente la pregunta de este capítulo. Sabemos qué distribuye el organismo en equilibrio; la pregunta es qué proceso momento a momento genera esa distribución.  En este capítulo usaremos una distinción implícita en los capítulos sobre ascenso de colina y sistemas de retroalimentación, que será desarrollada con mayor detalle en el capítulo sobre la ley del efecto relativo: la diferencia entre *variables de decisión* y *reglas de respuesta. Una variable de decisión es la magnitud que el organismo compara entre alternativas; una regla de respuesta es el procedimiento que convierte esa comparación en una acción. en el caso de la kinesis, la regla de decisión es el resultado de comparar el nivel de glucosa un instante anterior con el nivel de glucosa presente. La regla de respuesta es el cambio de dar maromas a nado recto cuando la diferencia detectada es positiva.  Los dos modelos que examinaremos,  la maximización momentánea y el mejoramiento,  pueden producir patrones agregados semejantes, pero proponen variables de decisión distintas. La maximización momentánea supone que el organismo compara probabilidades instantáneas de refuerzo; el mejoramiento supone que compara tasas locales de refuerzo. En ambos caso, la regla de elección es simple y local: escoger la opción con la mayor probabilidad instantánea de refuerzo, o  moverse en la dirección que reduzca la diferencia en las tasas locales de refuerzo, ambas, una forma de optimización. La diferencia en la variable de decisión,  será crucial para evaluar qué tipo de proceso puede generar la igualación global observada.
