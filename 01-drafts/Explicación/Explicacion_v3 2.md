...
David Marr propuso un marco complementario para analizar específicamente sistemas de procesamiento de información, articulado en tres niveles:
...
En los capítulos sobre ascenso de colina y sistemas de retroalimentación operaremos principalmente al nivel computacional (¿qué problema de orientación resuelven estos mecanismos?) y algorítmico (¿qué regla de decisión produce el comportamiento observado?). En los capítulos de Semestre II sobre aprendizaje por refuerzo y sus correlatos neurales, el nivel de implementación entrará al análisis. El marco de Marr permite leer cada tipo de modelo en el nivel correcto y evitar exigirle que responda preguntas para las que no fue diseñado.
