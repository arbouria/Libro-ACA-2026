...
Cada mañana, antes de comenzar su día, Wall-E hace lo mismo: conecta su panel solar y recarga su batería. Inmediatamente inicia una rutina de búsqueda: navega por un terreno accidentado, identifica objetos de interés entre la basura y regresa a su base antes de que se agote su energía. Este ritual aparentemente simple revela algo profundo. Wall-E enfrenta el problema fundamental que comparten todos los agentes adaptativos, desde bacterias hasta algoritmos de inteligencia artificial: cómo distribuir el comportamiento en el tiempo y en el espacio para maximizar la obtención de recursos necesarios para sobrevivir, funcionar o cumplir sus objetivos.

Ahora, imagina que te encargan diseñar un robot explorador similar a Wall-E para un planeta desconocido. Tu misión es construir un agente autónomo capaz de localizar recursos dispersos en un terreno accidentado, evitar peligros y encontrar fuentes de energía que le permitan recargar y gestionar su limitada reserva energética. Tienes un presupuesto limitado: sensores imperfectos con bajo cociente señal-ruido, un procesador con memoria finita y una batería que se agota con cada movimiento.

Para que el robot sobreviva, debes resolver cuatro problemas fundamentales:
...
Este es exactamente el problema que enfrentan todos los organismos vivos y que la evolución ha resuelto. Una rata buscando alimento, una bacteria navegando hacia glucosa, un estudiante decidiendo cuánto tiempo dedicar a cada materia—todos enfrentan el mismo desafío: cómo distribuir su comportamiento en el tiempo y en el espacio para maximizar la obtención de recursos necesarios para sobrevivir, reproducirse o cumplir sus objetivos. 

Este curso estudia las soluciones biológicas y computacionales a este problema. Pero antes de presentar esas soluciones, debemos confrontar un problema pedagógico previo: cómo organizarlas para maximizar comprensión.
...
Este libro adopta una estrategia diferente: ingeniería inversa del comportamiento adaptable. Asumiremos que el comportamiento observado es la solución a un problema de adaptación, y nuestro trabajo es descubrir el algoritmo que lo genera. Para cada fenómeno, preguntaremos no solo "¿qué hacen los organismos?" sino también:
...
Establecemos el marco teórico general: niveles de explicación, el problema de la adaptabilidad, y la teoría de la selección natural como primera solución. Establecemos tres preguntas fundamentales: ¿Qué problema resuelve un comportamiento? ¿Cómo funciona el mecanismo? ¿Por qué evolucionó? Sin este marco, los fenómenos parecen inconexos.

### Bloque I: Mecanismos Sin Integración de Historia (Capítulos 4-5)

Estudiamos dos mecanismos fundamentales que permiten adaptación en tiempo real sin requerir integración de experiencias pasadas: ascenso de colina (comparación sucesiva) y sistemas de retroalimentación (comparación simultánea). Estos son las "tuercas y tornillos" más básicas. Nuestro agente puede subir una colina hacia la luz o mantener su temperatura estable, pero vive completamente en el "ahora": sin memoria, sin predicción, puramente reactivo.

### Bloque II: El Problema del Conocimiento - Asignación de Crédito (Capítulos 6-10)

¿Cómo aprender qué señales predicen recursos importantes? Cuando aparece un suceso biológicamente importante (comida, peligro), ¿a cuál de los múltiples eventos, señales o acciones previas debe asignarse la responsabilidad de su presencia? Este es el problema de la asignación de crédito: ¿qué predice qué? Revisamos modelos clásicos de aprendizaje asociativo y sus extensiones contemporáneas, incluyendo modelos basados en teoría de la información y filtros bayesianos. Nuestro agente deja de ser puramente reactivo y aprende a predecir el futuro.

### Bloque III: El Problema de la Acción - Elección y Optimización (Capítulos 11-15)

Una vez que hemos aprendido qué predice qué, surge la pregunta central: ¿cómo usar ese conocimiento para distribuir óptimamente el comportamiento en el tiempo y en el espacio? Este es el problema de la elección bajo restricciones: el tiempo es finito, los comportamientos compiten entre sí, y las decisiones tienen costos de oportunidad. Estudiamos la ley del efecto, programas de refuerzo, la ley de igualación, y culminamos con modelos de optimización en equilibrio que integran economía conductual. Nuestro agente ahora conecta su conocimiento con la acción y aprende a elegir racionalmente.
...
Hasta ahora asumimos que el agente sabe con certeza en qué estado del mundo se encuentra. Ahora relajamos ese supuesto: estudiamos entornos volátiles y modelos bayesianos avanzados donde el agente debe razonar bajo incertidumbre profunda sobre el estado del mundo.

### Bloque VI: Estructura de Preferencias

Hasta ahora asumimos que ciertos sucesos tienen valor (comida es buena, dolor es malo). Pero ¿de dónde viene ese valor? ¿Por qué preferimos recompensas grandes sobre pequeñas? ¿Por qué descontamos recompensas futuras? Este bloque estudia los determinantes de la asignación de valor: probabilidad, demora y magnitud.

## Dos Orígenes de las Soluciones
...
Estos algoritmos no aparecen aislados. En los Bloques I-II estudiaremos algoritmos de comparación y retroalimentación. En el Bloque III, algoritmos de reducción de error y optimización. En el Bloque IV, algoritmos que balancean exploración y explotación. A lo largo del curso veremos cómo todos contribuyen a resolver el problema adaptativo fundamental.
...
Los estudiantes de hoy crecieron con algoritmos de recomendación, navegación GPS y juegos con IA. Tienen una intuición operativa sobre aprendizaje de máquinas que generaciones previas no tenían. Lo que les falta es un puente entre esa intuición informal y los principios formales.

Los simuladores interactivos, los ejemplos concretos y una narrativa de diseño construyen ese puente. Las matemáticas dejan de ser un obstáculo y se convierten en el lenguaje natural para expresar regularidades empíricas y algoritmos, lo que abre nuevas puertas de comprensión. El comportamiento adaptable no es un museo de curiosidades históricas. Es un campo vivo con aplicaciones en robótica, neurociencia computacional, economía conductual, psicología clínica, psiquiatría computacional y políticas públicas. Los mismos principios que explicamos aquí operan en algoritmos que determinan qué videos te recomienda YouTube, en modelos de neurociencia que estudian adicción, y en políticas de salud pública que buscan modificar comportamientos de riesgo.
