...
## Acciones, Inducción y el Problema del Control
...
## Thorndike y dos preguntas que se confundieron durante décadas
...
Su interpretación fue la *Ley del Efecto*: una respuesta seguida de un estado de cosas satisfactorio queda asociada con la situación y aumenta su probabilidad futura. La contigüidad temporal era, para Thorndike, tanto condición suficiente como necesaria.

Pero los datos contienen dos observaciones distintas. La primera: el gato *hace algo* al ser encerrado. No se queda inmóvil, no selecciona al azar de toda la gama posible de movimientos de un cuerpo en el espacio. Explora, araña, empuja, vocaliza —respuestas apropiadas a la situación de escape, no al cortejo ni a la alimentación. La segunda: de ese repertorio inicial, el animal selecciona progresivamente la respuesta que abre la caja. Son fenómenos distintos: uno sobre el origen de la variabilidad conductual, otro sobre su selección. La Ley del Efecto intentó responderlos con el mismo principio. Esa confusión costó décadas.

---

## El primer sesgo: la inducción de respuestas por el SBI
...
## El experimento de Skinner y la réplica de Staddon y Simmelhag
...
La interpretación de Skinner fue directa: para cada paloma, alguna respuesta había ocurrido accidentalmente inmediatamente antes de la entrega de comida, y esa contigüidad accidental era suficiente para fortalecerla. El animal no necesitaba detectar ninguna relación real entre su conducta y el resultado; bastaba la proximidad temporal para producir el aprendizaje. Skinner resumió su posición con una frase que se volvería célebre: decir que un reforzador es contingente sobre una respuesta no significa otra cosa que decir que se presenta después de la respuesta. Para él, la contigüidad temporal era el mecanismo, y el experimento de superstición era su demostración más limpia.
...
La primera conclusión es entonces esta: la contigüidad no es una condición *suficiente* para la asignación de crédito a respuestas.
...
Los resultados fueron concluyentes. Con demoras de hasta dieciséis segundos, el grupo experimental aprendió claramente; el grupo yoked no lo hizo. Con treinta y dos segundos, el experimental seguía respondiendo más, aunque más lento. A sesenta y cuatro segundos, ambos grupos fueron indistinguibles.

La figura 7.2 muestra esta disociación. Hay dos cosas en esos datos.
...
Y sin embargo, las palomas de Lattal aprendieron. La tasa de picoteo de la tecla aumentó claramente por encima de la línea base en cinco de los cinco sujetos del experimento, como muestra la figura 7.3. El animal aprendió a picar la tecla a pesar de que esa tecla nunca fue inmediatamente seguida de comida —a pesar de que, estrictamente hablando, el procedimiento estaba diseñado para que hubiera una separación garantizada.
...
La segunda conclusión es entonces esta: la contigüidad tampoco es una condición *necesaria* para la asignación de crédito a respuestas.
...
## Perspectiva molecular y perspectiva molar
...
Veremos esta distinción reaparecer con más fuerza cuando lleguemos al tema de los programas de refuerzo y la igualación, donde las teorías molares tienen predicciones distintas —y en muchos casos más precisas— que las teorías moleculares.

---

## La percepción de causalidad: el experimento de Killeen
...
## Por qué la contigüidad es un buen heurístico, y cuándo falla
...
## Apertura hacia los modelos formales

Todo lo que hemos discutido en este capítulo puede reformularse con una pregunta computacional precisa: ¿cómo actualiza un agente el valor que asigna a una acción en función de los resultados que ha obtenido en el pasado? Esa pregunta tiene una respuesta formal, y el siguiente capítulo la desarrolla a partir de la ecuación que Bush y Mosteller propusieron a mediados del siglo XX —varios años antes de los modelos de estímulos que veremos más adelante en el curso.

La idea central es sencilla: el valor que el agente asigna a una respuesta se actualiza en cada ocasión, y la magnitud de esa actualización depende de la discrepancia entre lo que ocurrió y lo que se esperaba. Si la respuesta produjo más SBI de lo anticipado, su valor sube; si produjo menos, baja. Lo que hace esta ecuación especialmente interesante es que puede leerse de más de una manera, y cada lectura ilumina un aspecto distinto del mecanismo. El siguiente capítulo examina esas lecturas con detalle, a partir de los datos que ya conocemos sobre el aprendizaje de respuestas.

---

## Simulador 7.1: Inducción temporal y respuestas interinas/terminales
...
```python
# Simulador 7.2: Aprendizaje de contingencia respuesta-SBI con gradiente de demora
# Requiere: numpy, matplotlib, random
...
# ─── Parámetros (modifícalos para explorar) ───────────────────────────────────
P_SBI_DADO_RESPUESTA  = 0.30  # P(SBI | respuesta en ese segundo)
P_SBI_SIN_RESPUESTA   = 0.05  # P(SBI | no hay respuesta)
TASA_APRENDIZAJE      = 0.10  # α: qué tan rápido se actualiza el valor (0 a 1)
DEMORAS               = [0, 2, 8, 32]  # Demoras a comparar (segundos)
N_SESIONES            = 30    # Número de sesiones de entrenamiento
SEGUNDOS_POR_SESION   = 120   # Duración de cada sesión (segundos)
# ──────────────────────────────────────────────────────────────────────────────

def simular_aprendizaje(p_contingente, p_no_contingente, alfa, demora, n_ses, seg_por_ses):
    """Simula el aprendizaje de una respuesta con una demora dada."""
    valor = 0.5  # Valor inicial neutral
    historia_valores = []
    historia_respuestas = []

    for sesion in range(n_ses):
        respuestas_sesion = 0
        for segundo in range(seg_por_ses):
            # Decisión de responder (probabilidad proporcional al valor actual)
            responde = random.random() < valor
            if responde:
                respuestas_sesion += 1
                # SBI con demora después de la respuesta
                if random.random() < p_contingente:
                    # Peso del SBI decrece con la demora
                    peso = np.exp(-demora / 8.0)  # gradiente exponencial
                    sbi = peso
                else:
                    sbi = 0
            else:
                # SBI no contingente
                sbi = 1.0 if random.random() < p_no_contingente else 0

            # Actualización del valor (regla delta)
            valor = valor + alfa * (sbi - valor)
            valor = max(0.01, min(0.99, valor))

        historia_valores.append(valor)
        historia_respuestas.append(respuestas_sesion / seg_por_ses)

    return historia_valores, historia_respuestas

fig, axes = plt.subplots(1, 2, figsize=(14, 5))
colores = ['#1f77b4', '#ff7f0e', '#2ca02c', '#d62728']

for i, demora in enumerate(DEMORAS):
    valores, tasas = simular_aprendizaje(
        P_SBI_DADO_RESPUESTA, P_SBI_SIN_RESPUESTA,
        TASA_APRENDIZAJE, demora, N_SESIONES, SEGUNDOS_POR_SESION
    )
    axes[0].plot(valores, color=colores[i], linewidth=2, label=f'Demora = {demora} seg')
    axes[1].plot(tasas, color=colores[i], linewidth=2, label=f'Demora = {demora} seg')
...
axes[0].set_ylabel('Valor asignado a la respuesta')
axes[0].set_title('Aprendizaje del valor según demora')
axes[0].axhline(y=0.5, color='gray', linestyle='--', alpha=0.5, label='Valor neutro')

axes[1].set_ylabel('Tasa de respuesta (por segundo)')
axes[1].set_title('Tasa de respuesta según demora')

plt.tight_layout()
plt.show()

# Tabla de valores finales
print("\nValores finales por condición de demora:")
print(f"{'Demora (seg)':<15} {'Valor final':<15} {'Interpretación'}")
print("-" * 55)
for demora in DEMORAS:
    v, _ = simular_aprendizaje(P_SBI_DADO_RESPUESTA, P_SBI_SIN_RESPUESTA,
                                TASA_APRENDIZAJE, demora, N_SESIONES, SEGUNDOS_POR_SESION)
    interpretacion = "Aprende bien" if v[-1] > 0.65 else ("Aprende parcialmente" if v[-1] > 0.55 else "No aprende")
    print(f"{demora:<15} {v[-1]:<15.3f} {interpretacion}")
```
...
### Hacia atrás: Asignación de crédito a estímulos (Capítulo 6)

La estructura argumentativa de este capítulo replica la del anterior. En el capítulo 6, la pregunta era si la contigüidad entre estímulo y SBI era condición necesaria y suficiente para el aprendizaje predictivo; la conclusión fue que no lo era en ninguna de las dos direcciones. Aquí la misma pregunta se formula para las respuestas, y la conclusión es la misma. El paralelismo no es casual: ambos problemas son instancias del problema de la asignación de crédito, y ambos se resuelven con el mismo mecanismo subyacente —la detección de contingencia estadística, modulada por sesgos inductivos que priorizan el espacio de búsqueda.

### Hacia adelante: La ecuación de Bush y Mosteller (Capítulo 8)
...
### Lateral: Programas de refuerzo e igualación (Bloque IV)
...
El capítulo siguiente formaliza el mecanismo de actualización que hace posible esta detección de contingencia, a partir de la ecuación que Bush y Mosteller propusieron para describir cómo el valor de una acción se ajusta con la experiencia.
