...
Para ver cómo funciona la primera forma, sigamos el ejemplo numérico de las diapositivas. Con $\alpha = 0.5$, $V_0 = 0$, y refuerzo constante $R = 1$:

| Ensayo | $R_t$ | $V_t$ | $V_{t+1} = 0.5\,V_t + 0.5\,R_t$ |
|--------|--------|--------|----------------------------------|
| 1      | 1      | 0.000  | 0.500                            |
| 2      | 1      | 0.500  | 0.750                            |
| 3      | 1      | 0.750  | 0.875                            |
...
Noten el patrón: en cada iteración, el $V$ obtenido en el ensayo previo se convierte en el nuevo $V_t$ del ensayo siguiente. El valor crece rápidamente al principio y se desacelera conforme se aproxima a 1. Con $\alpha = 0.2$ el proceso es más lento: $V_1 = 0.2$, $V_2 = 0.36$, $V_3 = 0.488$, $V_4 = 0.590$. La forma de la trayectoria es la misma; cambia la velocidad. La curva negativamente acelerada de las figuras 8.1 y 8.2 emerge directamente de esta dinámica.
...
La transición de la media simple a la regla delta tiene una derivación algebraica elegante. Partimos de la media en el ensayo $t$:

$$V_t = \frac{1}{t}\sum_{i=1}^{t} R_i$$

Y de la media en el ensayo $t-1$:

$$V_{t-1} = \frac{1}{t-1}\sum_{i=1}^{t-1} R_i$$

Multiplicando la segunda por $(t-1)$: $(t-1)V_{t-1} = \sum_{i=1}^{t-1} R_i$

Las dos ecuaciones comparten todos los términos excepto $R_t$. Sustituyendo:
...
Esta es la media simple expresada como regla de actualización: para calcular el promedio nuevo solo necesito el promedio anterior y el refuerzo actual. No necesito recordar ningún ensayo individual. Pero el peso $\frac{1}{t}$ decrece con cada ensayo —cada nueva observación importa menos que la anterior, lo cual no es deseable si el entorno puede cambiar.

La solución es reemplazar el peso decreciente $\frac{1}{t}$ por un peso *constante* $\alpha$:

$$V_t = V_{t-1} + \alpha\,(R_t - V_{t-1})$$

Que es exactamente la regla delta. Pero ahora podemos expandir recursivamente para ver qué peso recibe cada refuerzo pasado. Sustituyendo $V_{t-1}$ por su propia ecuación, luego $V_{t-2}$, y así sucesivamente:
...
**6.** Usa el Simulador 8.1 para explorar qué ocurre con el valor de equilibrio cuando la probabilidad de refuerzo es 0.5 en lugar de 1.0. ¿Hacia qué valor converge $V$? ¿Cambia ese valor de equilibrio al modificar $\alpha$? Explica por qué el valor de equilibrio depende de la probabilidad de refuerzo pero no de $\alpha$.

**7.** *(Reflexión)* El parámetro $\alpha$ determina el tamaño efectivo de la ventana temporal sobre la que el organismo promedia su experiencia. Describe dos situaciones —una en que un $\alpha$ grande sería adaptativo y una en que sería desadaptativo. ¿Qué propiedades del entorno determinan el valor óptimo de $\alpha$? ¿Debería ese valor ser fijo a lo largo de la vida de un organismo, o debería poder ajustarse?
